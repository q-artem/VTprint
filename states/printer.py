...
async def handle_get_pages(message: types.Message, bot: Bot, state: FSMContext, _):
    if not await validate_pages_ranges(message, _):
        return
    await message.answer(_("pages_ranges_all_pages_from_n_available").format(str(message.text), await count_pages(message.text), (await state.get_data())["pages_total"]))
    await state.clear()
